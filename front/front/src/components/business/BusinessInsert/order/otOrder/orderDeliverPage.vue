    ...
    <div v-for="info in order" :key="info.id"  style="width: auto;margin: 20px 0;border:1px solid gainsboro">
      ...
      <el-row :gutter="20" style="">
        <el-col :span="4" style="text-align: center">
          ...
          <el-image
            style=" height:150px"
            :src="urlHead+info.commodity.main_img"
          ></el-image>
        </el-col>
...
export default {
  name: 'orderDeliverPage',
  inject : ['reload'],
  created() {
    axios.get('/order/selectOrderByDeliveredByMerchantId').then(
      (res) => {
        this.order = res.data;
      }
    );
  ...
  methods : {
    editRs(id){
      ...
      this.subOr.order_delivered = "未送达";
