<template>
  <div>
    ...
    <div style="width: auto;margin: 0 10%;">
      <el-col :span="6" v-for="c in furniture" :key="c.id" style="padding: 10px">
        <div style="border:1px gainsboro solid ;height: 350px">
          <router-link target="_blank" :to="{path : '/buyFurniturePage' , query : {id : c.commodity_id}}" style="text-decoration:none">
...
export default {
  name: 'selectFurnitureIndexPage',
  components : {addShopCart},
  inject : ['reload'],
  props : ['keyword','ord'],
  created() {
    axios.get('/furniture/selectAllFurniture').then((res)=>{this.furniture=res.data})
  },
  methods : {
    goBack(){
      this.reload();
    ...
    sortN(){
      ...
      axios.get('/furniture/selFurniture',{params:{keyword:this.keyword,sort:this.sor}}).then(
        (res) => {
