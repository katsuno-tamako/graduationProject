  ...
  <div>
    ...
    <div style="width: auto;">
      <div style="height100px;width: auto;margin: 0 11%">
        ...
        <el-button @click="sortI" v-if="sor==='up'" size="mini" type="primary"><i class="el-icon-bottom"></i>价钱排序</el-button>
        <el-button @click="sortI" v-if="sor==='down'" size="mini" type="primary"><i class="el-icon-top"></i>价钱排序</el-button>
        ...
        <el-button @click="sortS" v-if="sor === 'sDown'" size="mini" type="primary">评分<i class="el-icon-top"></i></el-button>
        <el-button @click="sortS" v-if="sor === 'sUp'" size="mini" type="primary">评分<i class="el-icon-bottom"></i></el-button>
      </div>
    </div>

    <div style="width: auto;margin: 0 10%;">
      <el-col :span="6" v-for="c in phone" :key="c.id" style="padding: 10px">
        <div style="border:1px gainsboro solid ;height: 350px">
          <router-link target="_blank" :to="{path : '/buyPhonePage' , query : {id : c.commodity_id}}" style="text-decoration:none">
...
export default {
  name: 'selectPhoneCommodityPage',
  inject : ['reload'],
  props : ['keyword','ord'],
  components : {addShopCart},
  created() {
    axios.get('/phone/selectAllPhone').then((res)=>{this.phone=res.data})
  },
  methods : {
    goBack(){
      this.reload();
    ...
    sortN(){
      ...
      axios.get('/phone/selPhone',{params:{keyword:this.keyword,sort:this.sor}}).then(
        (res) => {
          this.phone = res.data;
        }
      );
