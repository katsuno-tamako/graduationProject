<template>
  <div>
    ...
    <div style="width: auto;margin: 0 10%;">
      <el-col :span="6" v-for="c in food" :key="c.id" style="padding: 10px">
        <div style="border:1px gainsboro solid ;height: 350px">
          <router-link target="_blank" :to="{path : '/buyFoodPage' , query : {id : c.commodity_id}}" style="text-decoration:none">
...
export default {
  name: 'selectFoodIndexPage',
  inject : ['reload'],
  props : ['keyword','ord'],
  components : {addShopCart},
  created() {
    axios.get('/food/selectAllFoodInfo').then((res)=>{this.food = res.data})
  },
  methods : {
    goBack(){
      this.reload();
    ...
    sortN(){
      ...
      axios.get('/food/selFood',{params:{keyword:this.keyword,sort:this.sor}}).then(
        (res) => {
          this.food = res.data;
        }
      );
