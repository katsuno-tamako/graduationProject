<template>
  <div>
    ...
    <div style="width: auto;margin: 0 10%;">
        <el-col :span="6" v-for="c in computer" :key="c.id" style="padding: 10px">
          <div style="border:1px gainsboro solid ;height: 350px">
            <router-link target="_blank" :to="{path : '/buyComputerPage' , query : {id : c.commodity_id}}" style="text-decoration:none">
...
export default {
  name: 'selectComputerCommodityPage',
  props : ['keyword','ord'],
  inject : ['reload'],

  created() {
    axios.get('/computer/selectComputer').then((res)=>{this.computer = res.data})
  },
  ...
  methods : {
    ...
    sortN(){
      ...
      axios.get('/computer/selComputer',{params:{keyword:this.keyword,sort:this.sor}}).then(
        (res) => {
